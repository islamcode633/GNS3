...
def getinfo_aboute_templates(token):
    ...
    response = requests.get(url=f'{ml.URL}{api}', headers=headers).content
    return ml.deserialize(response)
...
def create_node(project_id, token, vpcs_template_id=None, cloud_template_id=None):
    ...
    response = ml.post_method(api=api, headers=headers, data=data)
    if response.status_code == 201:
        return ml.deserialize(response.content)
...
def start_node(project_id, node_id, token):
    ...
    ml.deserialize(requests.post(url=f'{ml.URL}{api}', headers=headers, data='{}').content)


token = ml.get_token()
project_id = ml.create_project(token)
